<?php

declare(strict_types=1);

namespace DoctrineMigrations;

use Doctrine\DBAL\Schema\Schema;
use Doctrine\Migrations\AbstractMigration;

/**
 * Auto-generated Migration: Please modify to your needs!
 */
final class Version20251113184928 extends AbstractMigration
{
    public function getDescription(): string
    {
        return '';
    }

    public function up(Schema $schema): void
    {
        $connection = $this->connection;
        
        // Check if admin table exists before creating it
        $adminTableExists = $connection->executeQuery(
            "SELECT COUNT(*) FROM information_schema.tables 
             WHERE table_schema = DATABASE() 
             AND table_name = 'admin'"
        )->fetchOne();
        
        if ($adminTableExists == 0) {
            $this->addSql('CREATE TABLE `admin` (id INT AUTO_INCREMENT NOT NULL, email VARCHAR(255) NOT NULL, password VARCHAR(255) NOT NULL, name VARCHAR(255) NOT NULL, roles JSON NOT NULL, UNIQUE INDEX UNIQ_880E0D76E7927C74 (email), PRIMARY KEY(id)) DEFAULT CHARACTER SET utf8mb4 COLLATE `utf8mb4_unicode_ci` ENGINE = InnoDB');
        }
        
        // Check if customer table has password column before adding it
        $passwordColumnExists = $connection->executeQuery(
            "SELECT COUNT(*) FROM information_schema.columns 
             WHERE table_schema = DATABASE() 
             AND table_name = 'customer' 
             AND column_name = 'password'"
        )->fetchOne();
        
        if ($passwordColumnExists == 0) {
            $this->addSql('ALTER TABLE customer ADD password VARCHAR(255) NOT NULL');
        }
        
        // Check if customer table has roles column before adding it
        $rolesColumnExists = $connection->executeQuery(
            "SELECT COUNT(*) FROM information_schema.columns 
             WHERE table_schema = DATABASE() 
             AND table_name = 'customer' 
             AND column_name = 'roles'"
        )->fetchOne();
        
        if ($rolesColumnExists == 0) {
            $this->addSql('ALTER TABLE customer ADD roles JSON NOT NULL');
        }
        
        // Check if unique index on customer.email exists before creating it
        $indexExists = $connection->executeQuery(
            "SELECT COUNT(*) FROM information_schema.statistics 
             WHERE table_schema = DATABASE() 
             AND table_name = 'customer' 
             AND index_name = 'UNIQ_81398E09E7927C74'"
        )->fetchOne();
        
        if ($indexExists == 0) {
            $this->addSql('CREATE UNIQUE INDEX UNIQ_81398E09E7927C74 ON customer (email)');
        }
    }

    public function down(Schema $schema): void
    {
        // this down() migration is auto-generated, please modify it to your needs
        $this->addSql('DROP TABLE `admin`');
        $this->addSql('DROP INDEX UNIQ_81398E09E7927C74 ON customer');
        $this->addSql('ALTER TABLE customer DROP password, DROP roles');
    }
}
